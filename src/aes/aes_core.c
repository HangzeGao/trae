 ...
 *  Adapted from GmSSL reference implementation.
 */
...
int aes_set_encrypt_key(AES_KEY *aes_key, const uint8_t *key, size_t keylen)
{
	...
	case AES128_KEY_SIZE:
		aes_key->rounds = AES128_ROUNDS;
		break;
	case AES192_KEY_SIZE:
		aes_key->rounds = AES192_ROUNDS;
		break;
	case AES256_KEY_SIZE:
		aes_key->rounds = AES256_ROUNDS;
		break;
	default:
		return 0;
	}
...
/* GF(2^8) xtime: multiply by polynomial 'x' */
static uint8_t x2(uint8_t a)
{
	return (a >> 7) ? ((a << 1) ^ 0x1b) : (a << 1);
}

/* polynomial multiplication helper functions */
#define x1(a) (a)
#define x3(a) (x2(a) ^ x1(a))
#define x9(a) (x2(x2(x2(a))) ^ x1(a))
#define xb(a) (x2(x2(x2(a))) ^ x2(a) ^ x1(a))
#define xd(a) (x2(x2(x2(a))) ^ x2(x2(a)) ^ x1(a))
#define xe(a) (x2(x2(x2(a))) ^ x2(x2(a)) ^ x2(a))
...
/* inv_mix_columns using 0e 0b 0d 09 matrix (inverse of mix_columns) */
...
static void memxor_bytes(uint8_t *out, const uint8_t *a, const uint8_t *b, size_t len)
{
	while (len--) {
		*out++ = *a++ ^ *b++;
	}
	...
	const uint8_t *in, size_t nblocks, uint8_t *out)
{
	...
	memcpy(piv, iv, 16);
	while (nblocks--) {
		/* Save current ciphertext block BEFORE overwriting out (when in == out) */
		uint8_t curr[16];
		memcpy(curr, in, 16);
		aes_decrypt(key, curr, tmp);
		memxor_bytes(out, tmp, piv, 16);
		memcpy(piv, curr, 16);
		in += 16;
		out += 16;
	}
...
void aes_ctr32_encrypt(const AES_KEY *key, uint8_t ctr[16],
